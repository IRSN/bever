---
title: "nextpot Package: User's Guide"
author: "Yves Deville"
date: "`r Sys.Date()`"
output:
...
This document was created with **nextpot** **`r packageVersion("nextpot")`**.
As is the case for the whole package, this vignette is to be considered as a DRAFT
version that still needs, checking, renaming, ...

# Introduction

## Goals

This package is devoted to Extreme-Value models considered from the
classical frequentist perspective or from the Bayesian perspective. It
aims at producing classical graphics and diagnostics such as the
Return Level plots and at comparing different approaches and different
results. Although the package contains some functions dedicated to the
estimation and inference of these models, the results can also be
produced by using different R packages.
...
Statistics models do not fully describe the extreme observation
occurring within the blocks.  We can consider the `"GEV"` and
`"OS"` models as derived from a POT model, either `"PP"` or `"poisGP"`.
Indeed, we could consider `"GEV"` and "`OS"`models as resulting form a
temporal aggregation from a POT model as implemented in the `Renouv`
function of **Renext**. As a major difference between `"PP"` and
`"poisGP"`, the parameters of `"poisGP"` relate to the threshold $u$,
while those of `"PP"` do not.

The models above will in the package be considered both from a
...
| Model    | Freq. from data | Bayes from data | Bayes from MCMC|
|:---------|:-----------:|:---------:|:--------:|
|GEV| | |`GEVBayes0`|    
|Poisson-GP|`poisGPML`|`poisGPBayes`|`poisGPBayes0`|
...
Remind that the parameters of the EV models considered in **nextpot**
...
Since the **nextpot** package aims at "importing" MCMC iterates from
...
When using **nextpot**, this plot will be obtained by first calling
...
by ML, and the Bayesian credible intervals on quantiles are close to
the profile-likelihood confidence interval for the same level (here
$70\%$). Of course, the Bayesian RL plot would be different if an
informative prior had been specified.
...
show as a straight line. For POT models `PP`or `poisGP` a logarithmic scale
will be used because the theoretical RL curve corresponding to an exponential
distribution will show as a straight line.
...
return levels are described in Chap. 3 of Coles' book XXX.

We begin by fitting the data using ML. We will use the `TVGEV`
function of **NSGEV** in order to get the profile-likelihood
confidence intervals on the quantiles. Since this function is devoted
to time-varying models, it requires a "date" variable indicating the
beginning of the blocks. The RL plot is actually obtained by using the
`predict` method which here may be misleading.
	  
```{r portpirieML_RL1, message=FALSE}	   
...
posterior using `GEVBayes0` from **nextpot**. We reproduce here the
...
The plot can be compared to the one in Figure 3.5 in Coles' book XXX.
	
It can be seen that the frequentist and Bayesian results are very
...
**nextpot** will not use periods as tickmarks for the $x$-axis of the
predictive RL plot.
...
# XXX Questions on **nextpot** design
