...
output:
  rmarkdown::html_vignette:
    toc: true
vignette: >
  %\VignetteIndexEntry{The bever Package Goals and Uses}
...
difference is that the Bayesian RL plot from **nexpot** uses a *Gumbel
...
"sample points", we have to pass these when creating the `GEVBayes0`
object, using the `yMax` argument. We could have created an object
without giving `yMax` but it would the no longer have been possible to
show the sample points. Similarly, we could have used the `MAP` formal
argument, because the MAP is not computed by `GEVBayes0`.
